...
(function($) {
	...
	let bluResumePollTimer = null;
	let bluResumePollInFlight = false;
	let reservedBluPaymentWindow = null;
	...
	function storeBluResumeStateFromResponse(resume) {
		...
		const next = {
			...
			order_received_url: String(resume.order_received_url || ''),
	...
	function renderBluResumeCard(messageHtml) {
		if (!shouldShowBluResumeCard()) {
			stopBluResumePolling();
			$('#gstore-blu-resume-card').remove();
			$('[data-step="payment"]').removeClass('has-blu-waiting-panel');
			return;
		}

		const state = getBluResumeState();
		const $card = ensureBluResumeCardMount();
		if (!$card || !$card.length || !state) return;

		const orderId = parseInt(state.order_id, 10) || 0;
		const installments = parseInt(state.installments, 10) || 1;
		const totalText = getBluResumeTotalText();
		const status = normalizeBluOrderStatus(state.status || 'pending');
		const statusLabel = getBluResumeStatusLabel(status);
		const message = messageHtml || 'Conclua o pagamento na aba da Blu. Esta tela fica aberta para acompanhar a confirmacao.';

		$('[data-step="payment"]').addClass('has-blu-waiting-panel');
		$card
			.removeAttr('style')
			.attr('role', 'status')
			.attr('aria-live', 'polite')
			.html(`
				<div class="gstore-blu-waiting__header">
					<div>
						<div class="gstore-blu-waiting__eyebrow">${orderId ? `Pedido #${orderId}` : 'Pagamento Blu'}</div>
						<h3 class="gstore-blu-waiting__title">Aguardando confirmacao da Blu</h3>
					</div>
					<span class="gstore-blu-waiting__status" data-status="${escapeHtml(status)}">${escapeHtml(statusLabel)}</span>
				</div>
				<p class="gstore-blu-waiting__message">${message}</p>
				<div class="gstore-blu-waiting__summary">
					<div>
						<span>Total</span>
				...
				<div class="gstore-blu-waiting__actions">
					<a class="Gstore-btn Gstore-btn--submit gstore-blu-resume-card__continue" href="${escapeHtml(state.payment_url || '#')}" target="_blank" rel="noopener noreferrer">
						Abrir pagamento na Blu
					</a>
					<button type="button" class="Gstore-btn Gstore-btn--back gstore-blu-resume-card__change-installments">
						Trocar quantidade de parcelas
					</button>
					<a class="Gstore-btn Gstore-btn--back gstore-blu-resume-card__support" href="#" target="_blank" rel="noopener noreferrer" hidden>
						Falar com suporte
					</a>
				</div>
				<p class="gstore-blu-waiting__hint">Se a aba da Blu travar, volte aqui e abra o pagamento novamente. Nao feche esta tela ate confirmarmos o pedido.</p>
			`)
			.show();

		updateBluSupportAction($card);
		startBluResumePolling();
	}

	function normalizeBluOrderStatus(status) {
		return String(status || '').toLowerCase().trim();
	}

	function isBluPaidStatus(status) {
		return ['paid', 'success', 'confirmed', 'processing', 'completed'].indexOf(normalizeBluOrderStatus(status)) !== -1;
	}

	function isBluClosedStatus(status) {
		return ['expired', 'canceled', 'cancelled', 'failed', 'refunded'].indexOf(normalizeBluOrderStatus(status)) !== -1;
	}

	function getBluResumeStatusLabel(status) {
		const normalized = normalizeBluOrderStatus(status);
		if (isBluPaidStatus(normalized)) return 'Pagamento confirmado';
		if (isBluClosedStatus(normalized)) return 'Link encerrado';
		return 'Consultando status...';
	}

	function getBluResumeTotalText() {
		if (lastSummaryTotals && Number.isFinite(lastSummaryTotals.totalValue) && lastSummaryTotals.totalValue > 0) {
			return formatCurrency(lastSummaryTotals.totalValue);
		}
		const summary = lastCartSummaryData || lastNonEmptyCartSummaryData || {};
		return String(summary.total || summary.base_total || '');
	}

	function stopBluResumePolling() {
		if (bluResumePollTimer) {
			clearTimeout(bluResumePollTimer);
			bluResumePollTimer = null;
		}
	}

	function startBluResumePolling() {
		stopBluResumePolling();
		const state = getBluResumeState();
		if (!isBluResumeStateUsable(state) || isBluPaidStatus(state.status) || isBluClosedStatus(state.status)) return;
		bluResumePollTimer = setTimeout(function() {
			requestBluResumeStatus({ silent: true }).always(startBluResumePolling);
		}, 7000);
	}

	function updateBluResumeStateFromStatus(data) {
		...
		setBluResumeState(Object.assign({}, current, {
			...
			status: String(data.order_status || data.status || current.status || 'pending'),
			order_received_url: String(data.order_received_url || current.order_received_url || '')
		}));
	}

	function redirectToBluOrderReceived(state) {
		const target = state && state.order_received_url ? String(state.order_received_url) : '';
		clearBluResumeState();
		clearCheckoutDraftState();
		if (target) {
	...
	function requestBluResumeStatus(options) {
		const opts = options || {};
		const state = getBluResumeState();
		const deferred = $.Deferred();
		...
		if (bluResumePollInFlight) {
			deferred.reject();
			return deferred.promise();
		}

		bluResumePollInFlight = true;
		callBluResumeEndpoint('status', { force_check: 1 })
			.done(function(resp) {
				if (!(resp && resp.success && resp.data)) {
					if (!opts.silent) {
						showNotice('Nao foi possivel consultar o status do pagamento agora.', 'error');
					}
					deferred.reject();
					return;
				}
				...
				updateBluResumeStateFromStatus(data);
				const nextState = getBluResumeState();
				const nextStatus = normalizeBluOrderStatus(data.order_status || data.status || (nextState && nextState.status));

				if (data.status_check_failed) {
					if (!opts.silent) {
						showNotice('Nao foi possivel confirmar o status na Blu agora. Reabra o pagamento ou fale com o suporte.', 'error');
					}
					deferred.reject(data);
					return;
				}

				if (data.is_paid || isBluPaidStatus(nextStatus)) {
					redirectToBluOrderReceived(nextState);
			...
			.fail(function() {
				if (!opts.silent) {
					showNotice('Falha ao comunicar com a Blu agora. Tente novamente em instantes.', 'error');
				}
				deferred.reject();
			})
			.always(function() {
				bluResumePollInFlight = false;
			});
...
const subtotal = decodeHtmlEntities(stripHtmlText(it.subtotal || ''));
	...
	function getSelectedModeLabel(mode, labelsByMode) {
		...
		if (normalized === 'other') {
			const source = labelsByMode && labelsByMode.other ? labelsByMode.other : [];
			const labels = Array.isArray(source)
				? source
				: (source instanceof Set ? Array.from(source) : []);
			const uniqueLabels = Array.from(new Set(labels.map((label) => String(label || '').trim()).filter(Boolean)));
			return uniqueLabels.length === 1 ? uniqueLabels[0] : 'Outros';
		}
		if (normalized === 'melhor_envio') {
			return 'Melhor Envio';
		}
	...
	function getRateDeadlineHtml(rate) {
		...
		return `<small class="Gstore-checkout-item-shipping-option__deadline">${escapeHtml(text)}</small>`;
	}
	...
	function getRateCostHtml(rate) {
		...
		if (isRateQuoteNoticeActive(rate)) {
			const noticeMessage = rate.quote_notice_message || rate.other_note || '';
			if (noticeMessage) {
				return `<span class="gstore-shipping-quote-notice" style="display:block;max-width:100%;white-space:normal;overflow-wrap:anywhere;line-height:1.35;">${escapeHtml(noticeMessage)}</span>`;
			}
	...
	function getRateNoticeText(rate) {
		if (!rate) {
			return '';
		}
		if (normalizeRateMode(rate.mode) === 'other') {
	...
	function getSelectedModeNotice(mode, noticesByMode) {
		const normalized = normalizeRateMode(mode);
		const notices = noticesByMode && noticesByMode[normalized]
			? Array.from(noticesByMode[normalized]).filter(Boolean)
			: [];
	...
	function applyShippingSelectionToGroup(cartItemKey, rateId, mode, persistSelection) {
		...
		const sourceRates = checkoutShippingRatesByItem[cartItemKey] || [];
		const sourceRate = sourceRates.find((rate) => String(rate.rate_id || '') === selectedRateId);
		const memberKeys = isMelhorEnvioRate(sourceRate) && Array.isArray(sourceRate.applicable_cart_item_keys)
			? sourceRate.applicable_cart_item_keys.map(String)
			: getShippingGroupMemberKeys(cartItemKey);
		memberKeys.forEach((key) => {
			if (isMelhorEnvioRate(sourceRate)) {
				const matchingRate = (checkoutShippingRatesByItem[key] || []).find((rate) => String(rate.rate_id || '') === selectedRateId);
	...
	function leaveMelhorEnvioPackage(previousRate, sourceCartItemKey) {
		if (!isMelhorEnvioRate(previousRate) || !Array.isArray(previousRate.applicable_cart_item_keys)) {
			return;
		}
		previousRate.applicable_cart_item_keys.map(String).forEach((key) => {
			if (key === String(sourceCartItemKey)) return;
			const fallback = (checkoutShippingRatesByItem[key] || []).find((rate) => !isMelhorEnvioRate(rate));
			if (!fallback) return;
			checkoutSelectedShippingRateByItem[key] = String(fallback.rate_id || '');
			checkoutSelectedShippingByItem[key] = normalizeRateMode(fallback.mode) || 'land';
			persistSelectedRateForItem(key, checkoutSelectedShippingRateByItem[key]);
			persistShippingModeForItem(key, checkoutSelectedShippingByItem[key]);
		});
	}
	...
	function refreshCheckoutSelectedRateGroupRates(postcode, nonce, ajaxUrl) {
		...
		return Promise.all(groups.map((group) => {
			return fetchCheckoutRatesForGroup({
				...
				exclude_melhor_envio: String(group.rateId || '').indexOf('gstore_custom_shipping:melhor_envio:') === 0 ? 0 : 1,
			}, cleanCep, nonce, ajaxUrl)
				.then((rates) => {
...
function getInstallmentDisplayTotals(summaryData) {
	...
	function resolveUnifiedShippingMethodValue(items) {
		...
		if (allRateIds.size === 1) {
			...
			if (/^gstore_custom_shipping:(land|air|pickup|other|mixed)$/.test(rateId)) {
				return rateId;
			}
			if (rateId.indexOf('gstore_custom_shipping:service:') === 0) {
				return rateId;
			}
			if (rateId.indexOf('gstore_custom_shipping:melhor_envio:') === 0) {
				return rateId;
			}
					...
					const groupOptionsHtml = group.rates.map((rate) => {
						...
						return `
							<label class="Gstore-checkout-item-shipping-option${selectedClass}${noticeClass}"${noticeOptionStyle}>
								...
								<span class="Gstore-checkout-item-shipping-option__price${priceNoticeClass}"${noticePriceStyle}>${cost}${getRateDeadlineHtml(rate)}</span>
							</label>
						`;
					}).join('');
	...
	function renderShippingSummary(data) {
		...
		let selectedOtherTotal = 0;
		let selectedMelhorEnvioTotal = 0;
		const selectedModes = new Set();
		const selectedLabelsByMode = { land: new Set(), air: new Set(), melhor_envio: new Set(), pickup: new Set(), other: new Set() };
		const selectedNoticesByMode = { land: new Set(), air: new Set(), melhor_envio: new Set(), pickup: new Set(), other: new Set() };
		...
		if (hasItemRates && items.length) {
			...
			selectedRateGroups.forEach((group) => {
				...
				const selectedNotice = getRateNoticeText(groupedRate || group.rate);
				if (selectedNotice && selectedNoticesByMode[selectedModeForGroup]) {
					selectedNoticesByMode[selectedModeForGroup].add(selectedNotice);
				}
				...
				if (Number.isFinite(selectedCost)) {
					...
					} else if (selectedModeForGroup === 'other') {
						selectedOtherTotal += selectedCost;
					} else if (selectedModeForGroup === 'melhor_envio') {
						selectedMelhorEnvioTotal += selectedCost;
					} else {
			...
			if (selectedRate) {
				...
				const selectedNotice = getRateNoticeText(selectedRate);
				if (selectedNotice && selectedNoticesByMode[selectedMode]) {
					selectedNoticesByMode[selectedMode].add(selectedNotice);
				}
			}
			...
			if (selectedMode === 'air') {
				...
				selectedOtherTotal = selectedTotal;
			} else if (selectedMode === 'melhor_envio') {
				selectedMelhorEnvioTotal = selectedTotal;
			} else {
		...
		if (backendShippingValue > 0 && selectedModes.size <= 1 && Math.abs(backendShippingValue - selectedTotal) > 0.01) {
			...
			selectedOtherTotal = onlyMode === 'other' ? backendShippingValue : 0;
			selectedMelhorEnvioTotal = onlyMode === 'melhor_envio' ? backendShippingValue : 0;
		...
		if (subtotalValue > 0) {
			...
			lastSummaryTotals = {
				...
				selectedLabelsByMode: {
					...
					melhor_envio: Array.from(selectedLabelsByMode.melhor_envio),
					pickup: Array.from(selectedLabelsByMode.pickup),
					other: Array.from(selectedLabelsByMode.other),
				},
				selectedNoticesByMode: {
					land: Array.from(selectedNoticesByMode.land),
					air: Array.from(selectedNoticesByMode.air),
					melhor_envio: Array.from(selectedNoticesByMode.melhor_envio),
					pickup: Array.from(selectedNoticesByMode.pickup),
					other: Array.from(selectedNoticesByMode.other),
				},
				selectedLandTotal,
				selectedAirTotal,
				selectedPickupTotal,
				selectedOtherTotal,
				selectedMelhorEnvioTotal,
			};
		...
		`;
			...
			if (selectedModes.size > 1) {
				const modeRows = [
					{ key: 'land', label: getSelectedModeLabel('land', selectedLabelsByMode), value: selectedLandTotal, notice: getSelectedModeNotice('land', selectedNoticesByMode) },
					{ key: 'air', label: getSelectedModeLabel('air', selectedLabelsByMode), value: selectedAirTotal, notice: getSelectedModeNotice('air', selectedNoticesByMode) },
					{ key: 'melhor_envio', label: getSelectedModeLabel('melhor_envio', selectedLabelsByMode), value: selectedMelhorEnvioTotal, notice: getSelectedModeNotice('melhor_envio', selectedNoticesByMode) },
					{ key: 'pickup', label: getSelectedModeLabel('pickup', selectedLabelsByMode), value: selectedPickupTotal, notice: getSelectedModeNotice('pickup', selectedNoticesByMode) },
					{ key: 'other', label: getSelectedModeLabel('other', selectedLabelsByMode), value: selectedOtherTotal, notice: getSelectedModeNotice('other', selectedNoticesByMode) },
				];
				modeRows.forEach((row) => {
					...
					totalsHtml += `
						<div class="Gstore-checkout-shipping-totals__row Gstore-checkout-shipping-totals__row--shipping">
							...
							<span>${getShippingTotalValueHtml(row.key, row.value, row.notice)}</span>
						</div>
					`;
				});
			} else {
				...
				const singleValue = onlyMode === 'air'
					...
					: onlyMode === 'other'
							? selectedOtherTotal
							: onlyMode === 'melhor_envio'
								? selectedMelhorEnvioTotal
								: selectedLandTotal;
				const onlyModeLabel = getSelectedModeLabel(onlyMode, selectedLabelsByMode);
				const onlyModeNotice = getSelectedModeNotice(onlyMode, selectedNoticesByMode);
				totalsHtml += `
					<div class="Gstore-checkout-shipping-totals__row Gstore-checkout-shipping-totals__row--shipping">
						${getShippingTotalsLabelHtml(onlyModeLabel)}
						<span>${getShippingTotalValueHtml(onlyMode, singleValue, onlyModeNotice)}</span>
					</div>
				`;
			}
	...
	function updateOrderReviewTotals() {
		...
		const $subtotalRow = $table.find('.cart-subtotal').first();
			const $existing = $table.find('.gstore-shipping-land, .gstore-shipping-air, .gstore-shipping-melhor_envio, .gstore-shipping-pickup, .gstore-shipping-other');
			$existing.remove();

			const modes = lastSummaryTotals.selectedModes || [];
			const labelsByMode = lastSummaryTotals.selectedLabelsByMode || {};
			const noticesByMode = lastSummaryTotals.selectedNoticesByMode || {};
			let rowsHtml = '';
			if (modes.length > 1) {
				const modeRows = [
					{ key: 'land', label: getSelectedModeLabel('land', labelsByMode), value: lastSummaryTotals.selectedLandTotal || 0, notice: getSelectedModeNotice('land', noticesByMode) },
					{ key: 'air', label: getSelectedModeLabel('air', labelsByMode), value: lastSummaryTotals.selectedAirTotal || 0, notice: getSelectedModeNotice('air', noticesByMode) },
					{ key: 'melhor_envio', label: getSelectedModeLabel('melhor_envio', labelsByMode), value: lastSummaryTotals.selectedMelhorEnvioTotal || 0, notice: getSelectedModeNotice('melhor_envio', noticesByMode) },
					{ key: 'pickup', label: getSelectedModeLabel('pickup', labelsByMode), value: lastSummaryTotals.selectedPickupTotal || 0, notice: getSelectedModeNotice('pickup', noticesByMode) },
					{ key: 'other', label: getSelectedModeLabel('other', labelsByMode), value: lastSummaryTotals.selectedOtherTotal || 0, notice: getSelectedModeNotice('other', noticesByMode) },
				];
				modeRows.forEach((row) => {
					...
					rowsHtml += `
						<tr class="gstore-shipping-${row.key}">
							...
							<td>${getShippingTotalValueHtml(row.key, row.value, row.notice)}</td>
						</tr>
					`;
				});
				...
				const onlyValue = modeKey === 'air'
					...
					: modeKey === 'pickup'
						...
						: modeKey === 'other'
							? lastSummaryTotals.selectedOtherTotal
							: modeKey === 'melhor_envio'
								? lastSummaryTotals.selectedMelhorEnvioTotal
								: lastSummaryTotals.selectedLandTotal;
				rowsHtml += `
					<tr class="gstore-shipping-${modeKey}">
						<th>${getOrderReviewShippingLabelHtml(onlyLabel)}</th>
						<td>${getShippingTotalValueHtml(modeKey, onlyValue, getSelectedModeNotice(modeKey, noticesByMode))}</td>
					</tr>
				`;
			}
	...
	function calculateShipping(postcode) {
		...
		const preferredRateIdsBeforeCalculation = Object.assign({}, checkoutSelectedShippingRateByItem);
		const requests = groups.map(function(group) {
			...
			requestData.exclude_melhor_envio = 1;
			return fetchCheckoutRatesForGroup(requestData, cleanCep, nonce, ajaxUrl)
				.then(function(rates) {
					return { group: group, rates: rates };
				});
		});
		const allCartItemKeys = items.map(getCartItemKey).filter(Boolean);
		requests.push(
			fetchCheckoutRatesForGroup({ cart_item_keys: allCartItemKeys }, cleanCep, nonce, ajaxUrl)
				.then(function(rates) {
					return { isMelhorEnvioPackage: true, rates: rates };
				})
		);
		...
		Promise.all(requests).then(function(results) {
			...
			results.filter(function(result) { return !result?.isMelhorEnvioPackage; }).forEach(function(result) {
				if (result && result.rates && result.rates.length > 0) {
					const normalizedRates = result.rates
						.map(normalizeShippingRate)
						.filter(function(rate) { return rate.mode && rate.rate_id && !isMelhorEnvioRate(rate); });
					if (normalizedRates.length) hasAnyRates = true;
			...
			const packageResult = results.find(function(result) { return result?.isMelhorEnvioPackage; });
			const melhorEnvioRates = packageResult && Array.isArray(packageResult.rates)
				? packageResult.rates.map(normalizeShippingRate).filter(function(rate) { return rate.mode && rate.rate_id && isMelhorEnvioRate(rate); })
				: [];
			melhorEnvioRates.forEach(function(rate) {
				const applicableKeys = Array.isArray(rate.applicable_cart_item_keys) ? rate.applicable_cart_item_keys.map(String) : [];
				applicableKeys.forEach(function(key) {
					checkoutShippingRatesByItem[key] = (checkoutShippingRatesByItem[key] || []).filter(function(candidate) {
						return String(candidate.rate_id || '') !== String(rate.rate_id || '');
					}).concat([rate]);
				});
			});
			if (melhorEnvioRates.length) hasAnyRates = true;

			Object.keys(preferredRateIdsBeforeCalculation).some(function(key) {
				const preferredRateId = preferredRateIdsBeforeCalculation[key];
				const preferredRate = (checkoutShippingRatesByItem[key] || []).find(function(rate) {
					return String(rate.rate_id || '') === String(preferredRateId || '');
				});
				if (!isMelhorEnvioRate(preferredRate)) return false;
				applyShippingSelectionToGroup(key, preferredRate.rate_id, preferredRate.mode, false);
				return true;
			});
	...
	function bindEvents() {
		...
		$(document).on('change', 'input[name^="gstore_selected_shipping_rate["]', function() {
			...
			if (cartItemKey) {
				...
				const previousRateId = String(checkoutSelectedShippingRateByItem[cartItemKey] || '');
				const previousRate = (checkoutShippingRatesByItem[cartItemKey] || []).find(function(rate) {
					return String(rate.rate_id || '') === previousRateId;
				});
				applyShippingSelectionToGroup(cartItemKey, String(value || ''), normalizedMode, true);
				const selectedRate = (checkoutShippingRatesByItem[cartItemKey] || []).find(function(rate) {
					return String(rate.rate_id || '') === String(value || '');
				});
		...
		function maybeHandleBluResumeBeforeSubmit() {
			// ponytail: status/troca de parcelas agora vivem no painel; submit normal cria novo link.
			return false;
					...
					updateProcessingStep(3);
						...
						if (response && response.result === 'success') {
							if (isBluCheckoutSelected()) {
								saveCheckoutDraftState('blu_payment_waiting');
							} else {
	...
	function reserveBluPaymentWindow() {
		if (!isBluCheckoutSelected()) return null;
		if (reservedBluPaymentWindow && !reservedBluPaymentWindow.closed) {
			return reservedBluPaymentWindow;
		}

		try {
			reservedBluPaymentWindow = window.open('about:blank', '_blank');
			if (reservedBluPaymentWindow) {
				reservedBluPaymentWindow.opener = null;
				try {
					reservedBluPaymentWindow.document.title = 'Pagamento Blu';
					reservedBluPaymentWindow.document.body.innerHTML = '<p style="font-family:Arial,sans-serif;padding:24px;color:#111827;">Preparando pagamento seguro...</p>';
				} catch (e) {}
			}
		} catch (e) {
	...
	function closeReservedBluPaymentWindow() {
		if (!reservedBluPaymentWindow) return;
		try {
			if (!reservedBluPaymentWindow.closed) {
				reservedBluPaymentWindow.close();
			}
		} catch (e) {}
		reservedBluPaymentWindow = null;
	}
	...
	function openBluPaymentWindow(url, callback) {
		...
		let opened = null;
		if (reservedBluPaymentWindow && !reservedBluPaymentWindow.closed) {
			opened = reservedBluPaymentWindow;
			reservedBluPaymentWindow = null;
			try {
	...
	function showBluPaymentWaitingPanel(url, autoOpen) {
		if (!url) return;
		...
		const opened = autoOpen ? openBluPaymentWindow(url) : false;
		renderBluResumeCard(
			opened
				? 'Abrimos a Blu em uma nova aba. Conclua o pagamento por la; esta tela acompanha a confirmacao.'
				: 'Se a aba da Blu nao abriu automaticamente, toque em "Abrir pagamento na Blu".'
		);
	...
	function updateBluSupportAction($modal) {
		const $support = $modal.find('.Gstore-blu-checkout-modal__support, .gstore-blu-resume-card__support');
	...
	function openBluCheckoutModal(url) {
		if (!url) return;
		showBluPaymentWaitingPanel(url, false);
		return;
	...
	$(document).on('click', '.gstore-blu-resume-card__continue', function(e) {
		...
		const state = getBluResumeState();
		if (state && state.payment_url) {
			openBluPaymentWindow(String(state.payment_url));
			renderBluResumeCard('Pagamento aberto na Blu. Conclua por la; esta tela acompanha a confirmacao.');
	...
	$(document).on('click', '.gstore-blu-resume-card__change-installments', function(e) {
		e.preventDefault();
		const state = getBluResumeState();
		if (!isBluResumeStateUsable(state)) {
			showNotice('Nao foi possivel conferir o pedido pendente. Reabra o pagamento ou fale com o suporte.', 'error');
			return false;
		}

		setBluResumeCardBusy(true);
		requestBluResumeStatus({ silent: false })
			.done(function(data) {
				if (data && (data.is_paid || isBluPaidStatus(data.order_status || data.status))) {
					return;
				}
				...
				$(document.body).trigger('update_checkout');
				showNotice('Escolha a nova quantidade de parcelas e finalize o pedido novamente.', 'notice', 9000);
				setTimeout(function() {
					$('#gstore_blu_installments_select, #gstore_blu_installments').filter(':visible').first().trigger('focus');
				}, 250);
			})
			.always(function() {
				setBluResumeCardBusy(false);
