...
(function($) {
    ...
    // #region agent log - Interceptor de fetch: INJETA header readonly + monitora
    (function() {
        ...
        window.fetch = function(url, opts) {
            ...
            // Para TODAS as chamadas GET à Store API, injeta header X-Gstore-Readonly
            // Isso sinaliza ao PHP para NÃO salvar a sessão e NÃO deletar cookies
            if (urlStr.indexOf('wc/store') !== -1 && method.toUpperCase() === 'GET') {
                opts = opts || {};
                if (!opts.headers) {
                    opts.headers = { 'X-Gstore-Readonly': '1' };
                } else if (opts.headers instanceof Headers) {
                    opts.headers.set('X-Gstore-Readonly', '1');
            ...
            if (urlStr.indexOf('wc/store') !== -1 || urlStr.indexOf('wc-ajax') !== -1) {
                _origFetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'FETCH-INTERCEPT',message:'Requisicao WC detectada',data:{page:window.location.pathname,url:urlStr.substring(0,120),method:method,hasReadonlyHeader:!!(opts&&opts.headers&&(opts.headers['X-Gstore-Readonly']||(opts.headers instanceof Headers&&opts.headers.get('X-Gstore-Readonly'))))},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v4'})}).catch(function(){});
            }
            var result = _origFetch.apply(this, arguments);
            // Captura headers de diagnóstico da resposta REST
            if (urlStr.indexOf('wc/store/v1/cart') !== -1) {
                result.then(function(resp) {
                    var hd = {};
                    ['X-GD-CartCount','X-GD-SessClass','X-GD-Readonly','X-GD-Dirty','X-GD-CartInSess'].forEach(function(h){hd[h]=resp.headers.get(h);});
                    _origFetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'REST-RESPONSE-HEADERS',message:'Headers debug da resposta Store API',data:{page:window.location.pathname,url:urlStr.substring(0,120),headers:hd,status:resp.status},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v4'})}).catch(function(){});
                }).catch(function(){});
            }
            return result;
        };
        ...
        XMLHttpRequest.prototype.open = function(method, url) {
            ...
            if (urlStr.indexOf('wc/store') !== -1 || urlStr.indexOf('wc-ajax') !== -1) {
                fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'XHR-INTERCEPT',message:'Requisicao WC XHR detectada',data:{page:window.location.pathname,url:urlStr.substring(0,120),method:method},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v4'})}).catch(function(){});
            }
    ...
    function handleAddedToCart(event, fragments, cart_hash) {
        ...
        fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mini-cart-fix.js:handleAddedToCart',message:'added_to_cart disparado',data:{page:window.location.pathname,fragmentCount:_fragmentCount,domCount:getCurrentCountFromDom()},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v4'})}).catch(()=>{});
        // #endregion
    ...
    function init() {
        ...
        // Se o PHP mostra 0 mas localStorage tem itens, usa localStorage
            ...
            // #region agent log
            fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mini-cart-fix.js:init:restore',message:'Restaurando count de localStorage',data:{page:window.location.pathname,domCount:domCount,savedCount:savedCount,phpCount:phpCount},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v4'})}).catch(()=>{});
            // #endregion
            syncDOM(savedCount);
        }
