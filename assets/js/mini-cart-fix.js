...
(function($) {
    ...
    (function() {
        ...
        window.fetch = function(url, opts) {
            ...
            if (urlStr.indexOf('wc/store/v1/cart') !== -1) {
                result.then(function(resp) {
                    ...
                    _origFetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'REST-RESPONSE-HEADERS',message:'Headers debug da resposta Store API',data:{page:window.location.pathname,url:urlStr.substring(0,120),headers:hd,status:resp.status},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v6'})}).catch(function(){});
                }).catch(function(){});
            }
            return result;
        };
        ...
        XMLHttpRequest.prototype.open = function(method, url) {
            ...
            if (urlStr.indexOf('wc/store') !== -1 || urlStr.indexOf('wc-ajax') !== -1) {
                fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'XHR-INTERCEPT',message:'Requisicao WC XHR detectada',data:{page:window.location.pathname,url:urlStr.substring(0,120),method:method},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v6'})}).catch(function(){});
            }
    ...
    function handleAddedToCart(event, fragments, cart_hash) {
        ...
        fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mini-cart-fix.js:handleAddedToCart',message:'added_to_cart disparado',data:{page:window.location.pathname,fragmentCount:_fragmentCount,domCount:getCurrentCountFromDom()},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v6'})}).catch(()=>{});
        // #endregion
    ...
    function init() {
        ...
        // Se o PHP mostra 0 mas localStorage tem itens, usa localStorage
            ...
            // #region agent log
            fetch('http://127.0.0.1:7246/ingest/82530f36-f41c-4a9b-9141-c3c4bf366209',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'mini-cart-fix.js:init:restore',message:'Restaurando count de localStorage',data:{page:window.location.pathname,domCount:domCount,savedCount:savedCount,phpCount:phpCount},timestamp:Date.now(),sessionId:'debug-session',runId:'post-fix-v6'})}).catch(()=>{});
            // #endregion
            syncDOM(savedCount);
        }
